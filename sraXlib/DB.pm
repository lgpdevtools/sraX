...
sub get_publ_db {
	...
	open(FASTA_DNA,">$amr_db_fasta_dna") || die sraXlib::Functions::print_errf($amr_db_fasta_dna,"o");
	open(FASTA_RNA,">$amr_db_fasta_rna") || die sraXlib::Functions::print_errf($amr_db_fasta_rna,"o"); 
	open(FASTA_AA,">$amr_db_fasta_aa") || die sraXlib::Functions::print_errf($amr_db_fasta_aa,"o");
	...
	if (!$g_card->is_success){
		...
		if(sraXlib::Functions::check_file($amr_db_fasta_dna) == 1){
		...
		if(sraXlib::Functions::check_file($amr_db_fasta_rna) == 1){
			print STDOUT "\tTotal number of non-redundant RNA sequences: ";
			system("grep '>' $amr_db_fasta_rna | wc -l");
		}

		if(sraXlib::Functions::check_file($amr_db_fasta_aa) == 1){
